  ...
  */
  var WordFindGame = function (puzzleEl, options) {
    ...
    */
    ...
    // Game state
    var startSquare, selectedSquares = [], curOrientation, curWord = '', isComplete = false;
    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          $('.selected').removeClass('hint-letter').addClass('found');
          wordList.splice(i,1);
          $('input.word').filter(function () {
            return this.value.toLowerCase() === curWord;
          }).addClass('wordFound');
          if (typeof options.onWordFound === 'function') {
            options.onWordFound(curWord);
          }
        }
    ...
    */
    this.solve = function() {
      ...
      for( var i = 0, len = solution.length; i < len; i++) {
        ...
        var wordEl = $('input.word').filter(function () {
          return this.value.toLowerCase() === word;
        });
        if (!wordEl.hasClass('wordFound')) {
          for (var j = 0, size = word.length; j < size; j++) {
            var nextPos = next(x, y, j);
            $('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]').removeClass('hint-letter').addClass('solved');
          }

          wordEl.addClass('wordFound');
          if (typeof options.onWordFound === 'function') {
            options.onWordFound(word);
          }
        }
      }
    ...
    this.hintLetter = function(word) {
      var targetWord = String(word || '').toLowerCase();
      var solution = wordfind.solve(puzzle, [targetWord]).found[0];
      if (!solution) return false;

      var next = wordfind.orientations[solution.orientation];
      for (var letterIndex = 0; letterIndex < targetWord.length; letterIndex++) {
        var nextPos = next(solution.x, solution.y, letterIndex);
        var $cell = $('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]');
        if (!$cell.hasClass('hint-letter') && !$cell.hasClass('found') && !$cell.hasClass('solved')) {
          $cell.addClass('hint-letter');
          return true;
        }
      }
      return false;
